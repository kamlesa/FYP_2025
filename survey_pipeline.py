...
#                     tqdm, requests, python-dotenv, torch, numpy, openpyxl)
#   .env with:
...
def read_input_file(path: str) -> pd.DataFrame:
    ...
    Normalizes header whitespace to reduce mismatch issues.
    """
    path = path.strip()
    abs_path = os.path.abspath(path)
    if not os.path.exists(path):
        here = os.getcwd()
        nearby = [p for p in os.listdir(".") if p.lower().endswith((".csv", ".xlsx", ".xls"))]
        hint = "\n".join(f" - {p}" for p in sorted(nearby)) or " - (no .csv/.xlsx files found here)"
        raise FileNotFoundError(
            f"\nCould not find: {abs_path}\nWorking directory: {here}\nNearby data files:\n{hint}\n"
            f'Tip: run with a full path, e.g.\n  python survey_pipeline.py --input "C:\\path\\to\\file.xlsx"\n'
        )
...
# Excel headers vs legacy CSV headers
OPEN_ENDED_COLS_EXCEL = [
    "In your opinion, what are the main benefits of AI in law enforcement?",
    "What concerns, if any, do you have about the use of AI in law enforcement?",
    "What kinds of rules or safeguards would make you feel more comfortable with the use of AI in policing?",
]
OPEN_ENDED_COLS_CSV_LEGACY = ["Q9_Open", "Q10_Open", "Q11_Open"]

def choose_open_cols(df: pd.DataFrame):
    cols = list(df.columns)

    if all(c in cols for c in OPEN_ENDED_COLS_EXCEL):
        return OPEN_ENDED_COLS_EXCEL
    if all(c in cols for c in OPEN_ENDED_COLS_CSV_LEGACY):
        return OPEN_ENDED_COLS_CSV_LEGACY

    def norm(s): return re.sub(r"\s+", " ", s.strip().lower())
    norm_map = {norm(c): c for c in cols}

    excel_targets = [norm(c) for c in OPEN_ENDED_COLS_EXCEL]
    if all(t in norm_map for t in excel_targets):
        return [norm_map[t] for t in excel_targets]

    legacy_targets = [norm(c) for c in OPEN_ENDED_COLS_CSV_LEGACY]
    if all(t in norm_map for t in legacy_targets):
        return [norm_map[t] for t in legacy_targets]

    raise ValueError(
        "Could not find the three open-ended columns.\n"
        f"Excel headers expected:\n  {OPEN_ENDED_COLS_EXCEL}\n"
        f"Legacy CSV headers:\n  {OPEN_ENDED_COLS_CSV_LEGACY}\n\n"
        f"Columns present:\n  {cols}"
    )
...
# ---------------------------
def main():
    parser = argparse.ArgumentParser(description="Process survey responses (CSV or Excel).")
    parser.add_argument("-i", "--input", default=INPUT_PATH, help="Path to input file (.csv, .xlsx)")
    parser.add_argument("--dry-run", action="store_true", help="Only load and validate columns, then exit.")
    args = parser.parse_args()

    # 1) Load file
    df = read_input_file(args.input)

    # 2) Choose open-ended columns
    OPEN_ENDED_COLS = choose_open_cols(df)

    if args.dry_run:
        print("Dry run OK ✅")
        print(f"- File: {os.path.abspath(args.input)}")
        print(f"- Rows: {len(df)}")
        print(f"- Using open-ended columns:\n  {OPEN_ENDED_COLS}")
        return

    # 3) Clean open-ended text
    ...
    df["text_all_clean"] = df["text_all"]

    # 4) Preserve structured fields if present
    keep_cols = [c for c in PRESERVE_COLS if c in df.columns]

    # 5) Sentiment
    if not DEEPSEEK_API_KEY:
        print("WARNING: DEEPSEEK_API_KEY not set. Using stub sentiment labels.")
    ...
    # 8) Aggregate concern frequencies
    df["text_all_concerns"] = df.get("text_all_concerns", pd.Series([{}]*len(df))).apply(
